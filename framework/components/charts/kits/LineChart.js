...
import { Circle, G, Path, Polygon, Polyline, Rect, Svg,Text,Line } from "react-native-svg";
...
var LineChart = /** @class */ (function (_super) {
    ...
    function LineChart() {
        ...
        _this.renderDots = function (_a) {
            var status=_a.status, data = _a.data, index=_a.index,width = _a.width, height = _a.height, paddingTop = _a.paddingTop, paddingRight = _a.paddingRight, onDataPointClick = _a.onDataPointClick;
            var unit =_a.unit;
            var output = [];
            let labels = _a.labels;
      ...
      //      console.log(baseHeight)
            ...
            data.forEach(function (dataset) {
                ...
                dataset.data.forEach(function (x, i) {
                  //  console.log(i + " Status=" + status[i])
                    if (hidePointsAtIndex.includes(i)) {
                        return;
                    }
                    var rectWidth = 100
                    var cx = 20+paddingRight + (i * (width - paddingRight)) / dataset.data.length;
                    var cy = ((baseHeight - _this.calcHeight(x, datas, height)) / 4) * 3 +
                        paddingTop;
                    var onPress = function() {
                      _this.setState({focus:i==_this.state.focus?-1:i})
                    }.bind(_this);
                    let fill = {fill:'rgba(0, 106, 183, 1)',fontSize:10};
                    /*
                    150(none/unknown), 151(abnormal), 152(normal), 153(predicting),
                    160(pause), 161(unbusiness), 162(defrost)
                    */
                    //if(index == i)fill = {fill:'rgba(245, 120, 72, 1)',fontSize:10};
                    console.log("Status="+status[i])
                    if(i==_this.state.focus){
                      output.push(<Circle key={Math.random()} cx={cx} cy={cy} fill={"#00000033"} onPress={onPress} r={8}/>)
                    }
                    output.push(<Circle key={Math.random()} cx={cx} cy={cy} fill={"transparent"} onPress={onPress} r={20}/>)
                    if(status[i]!= -9999){
                      if(status[i]==152 || status[i]==150){
                        output.push(<Circle key={Math.random()} cx={cx} cy={cy} fill={"#7D7D7D"} onPress={onPress} r={5}/>)
                        output.push(<Circle key={Math.random()} cx={cx} cy={cy} fill={"#fff"} onPress={onPress} r={3}/>)
                      }
                      else if(status[i]==160){
                        output.push(<Circle key={Math.random()} cx={cx} cy={cy} fill={"#D5B142"} onPress={onPress} r={5}/>)
                        output.push(<Circle key={Math.random()} cx={cx} cy={cy} fill={"#fff"} onPress={onPress} r={3}/>)
                      }
                      else if(status[i]==162||status[i]==164){
                        output.push(<Circle key={Math.random()} cx={cx} cy={cy} fill={"#7D7D7D"} onPress={onPress} r={5}/>)
                        output.push(<Circle key={Math.random()} cx={cx} cy={cy} fill={"#fff"} onPress={onPress} r={3}/>)
                      }
                      else if(status[i]==163){
                        output.push(<Circle key={Math.random()} cx={cx} cy={cy} fill={"#2FA8FF"} onPress={onPress} r={5}/>)
                        output.push(<Circle key={Math.random()} cx={cx} cy={cy} fill={"#fff"} onPress={onPress} r={3}/>)
                      }
                      else if(status[i]==999){
                        output.push(<Circle key={Math.random()} cx={cx} cy={cy} fill={"#CA4940"} onPress={onPress} r={7}/>)
                        output.push(<Circle key={Math.random()} cx={cx} cy={cy} fill={"#fff"} onPress={onPress} r={5}/>)
                        output.push(<Circle key={Math.random()} cx={cx} cy={cy} fill={"#CA4940"} onPress={onPress} r={4}/>)
                      }
                      else {
                        output.push(<Circle key={Math.random()} cx={cx} cy={cy} fill={"#CA4940"} onPress={onPress} r={5}/>)
                        output.push(<Circle key={Math.random()} cx={cx} cy={cy} fill={"#fff"} onPress={onPress} r={3}/>)
                    ...
                    output.push(<Line key={Math.random()} x1={cx} y1={height+29} x2={cx} y2={height+23}
                ...
                dataset.data.forEach(function (x, i) {
                  //  console.log(i + " Status=" + status[i])
                    ...
                    var cy = ((baseHeight - _this.calcHeight(x, datas, height)) / 4) * 3 +
                        paddingTop;
                    if(i==_this.state.focus){
                      output.push(<Line key={Math.random()} x1={cx} y1={height+30} x2={cx} y2={0}
                      {..._this.getPropsForBackgroundLines()}
                      stroke ={"#7d7d7d"}
                      strokeDasharray={"3,3"}
                      />)
                    ...
                    if(i==_this.state.focus){
                      let value = x;
                      if(_a.binary){
                        x = x==0?LangUtil.getStringByKey("switch_off"):LangUtil.getStringByKey("switch_on")
                      }
                      output.push(<Rect width={150} height={45} rx={4} ry={4} x={i>(dataset.data.length/2)?(cx -150-5) :cx + 5} y={10}  fill="#6E6E6E"/>)
                      output.push(<Text
                                      x={i>(dataset.data.length/2)?(cx -145) :cx +15}
                                      y={25}
                                      fill="white"
                                      fontSize="11"
                                      textAnchor="start">
                    ...
                    /*
                    output.push(<Circle key={Math.random()} cx={cx} cy={cy} fill={"#f00"} onPress={onPress} r={4}/>)
                    output.push(<Circle key={Math.random()} cx={cx} cy={cy} fill={"#fff"} onPress={onPress} r={3}/>)
                    */
                });
        ...
        _this.renderShadow = function (_a) {
            ...
            return data.map(function (dataset, index) {
                return (<Polygon key={Math.random()} points={dataset.data
                    .map(function (d, i) {
                    var x = paddingRight +
        ...
        _this.renderStdLine = function (_a) {
      //     console.log("To Render STD LINE")
            ...
            if(std){
              std.map(function(item){
                let shift =  _this.calcHeight(item, datas, height);
                if(shift>=0 ){
                  var y1 = ((baseHeight - _this.calcHeight(item, datas, height)) / 4) * 3 +  paddingTop;
                  output.push(<Line key={Math.random()} x1={paddingRight} y1={y1} x2={width-20} y2={y1}
                  {..._this.getPropsForBackgroundLines()}
                  stroke ={"rgba(245, 120, 72, 1)"}
                  strokeDasharray={"2, 2"}
                  />)
                }
        ...
        _this.renderLine = function (_a) {
            var status=_a.status,width = _a.width, height = _a.height, paddingRight = _a.paddingRight, paddingTop = _a.paddingTop, data = _a.data, linejoinType = _a.linejoinType;
            ...
            data.forEach(function (dataset, index) {
                ...
                var points = dataset.data.map(function (d, i) {
                    if (d === null)
                        return lastPoint;
                    var x = (i * (width - paddingRight)) / dataset.data.length + paddingRight+20;
                    var y = ((baseHeight - _this.calcHeight(d, datas, height)) / 4) * 3 +
                        paddingTop;
                ...
                */
                ...
                dataset.data.forEach((d, i) => {
                  console.log("Point Status="+status[i]+ " data="+d)
                  if(i < dataset.data.length-1){
                    if(_a.binary &&  dataset.data[i+1]!=d){
                      var x = (i * (width - paddingRight)) / dataset.data.length + paddingRight+20;
                      var y = ((baseHeight - _this.calcHeight(d, datas, height)) / 4) * 3 +
                          paddingTop;
                      var x1 = ((i+1)* (width - paddingRight)) / dataset.data.length + paddingRight+20;
                      var y1 = ((baseHeight - _this.calcHeight(dataset.data[i+1], datas, height)) / 4) * 3 +
                              paddingTop;
                      var points = [

                      ]
                      if(d == 0){
                        points.push(x + "," + y, x1 + "," + y)
                        points.push(x1 + "," + y, x1 + "," + y1)
                      }
                      else{
                        points.push(x + "," + y, x + "," + y1)
                        points.push(x + "," + y1, x1 + "," + y1)
                      }

                      if(status[i]!= -9999 && status[i+1]!= -9999){
                        if(status[i]==150 || status[i]==152){
                          output.push(<Polyline key={Math.random()}
                             strokeLinejoin={linejoinType}
                             points={points.join(" ")}
                              fill="none" stroke={"#7D7D7D"}
                              strokeWidth={2}
                              strokeDasharray={dataset.strokeDashArra}
                              strokeDashoffset={dataset.strokeDashOffset}/>);
                          //output.push(<Circle key={Math.random()} cx={cx} cy={cy} fill={"#7D7D7D"} onPress={onPress} r={4}/>)
                        //  output.push(<Circle key={Math.random()} cx={cx} cy={cy} fill={"#fff"} onPress={onPress} r={3}/>)
                        }
                        else if(status[i]==160){
                          output.push(<Polyline key={Math.random()}
                             strokeLinejoin={linejoinType}
                             points={points.join(" ")}
                              fill="none" stroke={"#D5B142"}
                              strokeWidth={2}
                              strokeDasharray={"5,5"}
                              strokeDashoffset={dataset.strokeDashOffset}/>);
                        //  output.push(<Circle key={Math.random()} cx={cx} cy={cy}  stroke ={"#D5B142"}
                            //  strokeDasharray={"2, 2"}  onPress={onPress} r={4}/>)
                        //  output.push(<Circle key={Math.random()} cx={cx} cy={cy} fill={"#fff"} onPress={onPress} r={3}/>)
                        }
                        else if(status[i]==162||status[i]==164){
                          output.push(<Polyline key={Math.random()}
                             ...
                             points={points.join(" ")}
                              ...
                              strokeDashoffset={dataset.strokeDashOffset}/>);
                          //output.push(<Circle key={Math.random()} cx={cx} cy={cy} fill={"#2FA8FF"} onPress={onPress} r={4}/>)
                        //  output.push(<Circle key={Math.random()} cx={cx} cy={cy} fill={"#fff"} onPress={onPress} r={3}/>)
                        }
                        else if(status[i]==163){
                          output.push(<Polyline key={Math.random()}
              ...
              //   console.log("Point="+status[i])

                /*
                output.push(<Polyline key={Math.random()}
                   strokeLinejoin={linejoinType}
                   points={points.join(" ")}
                    fill="none" stroke={_this.getColor(dataset, 0.2)}
        ...
        _this.renderBezierLine = function (_a) {
            ...
            return data.map(function (dataset, index) {
                ...
                return (<Path key={Math.random()} d={result} fill="none" stroke={_this.getColor(dataset, 0.2)} strokeWidth={_this.getStrokeWidth(dataset)} strokeDasharray={dataset.strokeDashArray} strokeDashoffset={dataset.strokeDashOffset}/>);
            });
        };
        _this.renderBezierShadow = function (_a) {
            ...
            return data.map(function (dataset, index) {
                ...
                return (<Path key={Math.random()} d={d} fill={"url(#fillShadowGradient" + (useColorFromDataset ? "_" + index : "") + ")"} strokeWidth={0}/>);
            });
        };
    ...
    LineChart.prototype.render = function () {
        ...
        var legendOffset = 0;
        return (<View>
        <Svg onPress={()=>{console.log("Click");this.setState({focus:-1})}}
        height={height} width={width-10}>
          <Rect
          width={width-100} height={height + legendOffset} rx={0} ry={0} fill="transparent"
          fillOpacity={transparent ? 0 : 1}/>
          <G x="0" y={legendOffset}>
            {this.renderDefs(__assign(__assign(__assign({}, config), chartConfig), { data: data.datasets }))}
            <G>{this.renderHorizontalLine(__assign(__assign({}, config), { paddingTop: paddingTop,
                  paddingRight: paddingRight }))}
            </G>
            <G>{this.renderHorizontalLines(__assign(__assign({}, config), { paddingTop: paddingTop,
                  paddingRight: paddingRight }))}
            </G>
            <G>
            ...
            <G>
              {withVerticalLines &&
            (withInnerLines
                ? this.renderVerticalLines2(__assign(__assign({}, config), { data: data.datasets[0].data, paddingTop: paddingTop, paddingRight: paddingRight }))
                : withOuterLines
            ...
            <G>
              {withVerticalLabels &&
            this.renderVerticalLabels(__assign(__assign({}, config), { index:index, labels: labels, paddingTop: paddingTop, paddingRight: paddingRight, formatXLabel: formatXLabel }))}
            </G>
            <G>
              {this.renderLine(__assign(__assign(__assign({}, config), chartConfig), { binary:binary,index:index,status:status, paddingRight: paddingRight, paddingTop: paddingTop, data: data.datasets }))}
            </G>
            <G>
              {withDots &&
            this.renderDots(__assign(__assign({}, config), { labels: labels,binary:binary,index:index, unit:unit,status:status,data: data.datasets, paddingTop: paddingTop, paddingRight: paddingRight, onDataPointClick: onDataPointClick }))}
            </G>
            <G>
              {withScrollableDot &&
            this.renderScrollableDot(__assign(__assign(__assign({}, config), chartConfig), { index:index, data: data.datasets, paddingTop: paddingTop, paddingRight: paddingRight, onDataPointClick: onDataPointClick,

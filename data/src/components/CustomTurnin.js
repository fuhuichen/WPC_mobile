...
@inject('store')
@observer
export default class CustomTurnin extends Component {
  constructor(props) {
    super(props);
    this.props = props;
    var d = new Date();
  ...
  getWidgets(typeIndex,range) {
    ...
    if(typeIndex ==1 && (range == 'mm' || range == 'yyyy')){
      list.push(widgetList[1]);
    } else {
      list.push(widgetList[1]);
      list.push(widgetList[0]);
      list.push(widgetList[9]);
    }
  ...
  fetchData() {
    console.log("CustomTurnin fetchData");
  ...
  promiseRequests(promises){
    ...
    var doFail = function() {
      ...
      this.setState({loading:false,loading_err:false});
    }.bind(this)
  ...
  handleResults2(results){
    ...
    for(var k in results){
      ...
      if(widgetList[k].title== "bi_turnin_rate") {
        for(var x in d) {
          dataset_turningRate.push(d[x].row[0]);
          var datas = results[k].retrived[1].data;
          for(var n in datas) {
            dataset_walkby.push(datas[n]['walkby'].row[0]);
          }
        ...
        for(var x in d) {
          dataset_walkin.push(d[x].row[0]);
        }
      } else if(widgetList[k].title == '天氣') {
        this.getWeatherInfo(results[k]);
      }
      var list = [];
      if( range == 'mm') {
        for(var n in d) {
          data.datasets.push({data:d[n].row,max:100});
        }
      }else if(range == 'yyyy'){
        ...
        for(var n in d) {
          ...
          list.push(dp);
          if( n==0 ) {
            //data.datasets.push({data:list,max:100});
        ...
        if( k==0 ) {
          ...
          for(var i in dataset_walkin) {
            if(dataset_walkin[i] > 0) {
              if(dataset_turningRate[i] > 0) {
                tmpData.push(parseInt(dataset_walkin[i] / dataset_turningRate[i]));
              }
            } else if(dataset_walkin[i] == 0) {
              tmpData.push(dataset_walkby[i]);
            } else {
              tmpData.push(dataset_walkin[i])
            }
          }
          data.barDataset2 = tmpData;
          */
        } else if( k==2 ) {
          data.barDataset2=list;
        }
  ...
  handleResults(results){
    ...
    for(var k in results){
      ...
      var d = DataHandler.parseDataResponse(results[k], widgetList[k].data_source);
      if(widgetList[k].title == "bi_turnin_rate") {
      //  console.log("Turnin Rate*************")

        /*
        console.log(d[0].row)
        for(var n in d[0].row) {
          dataset_turningRate.push(d[0].row[n]);
        }
        var datas = results[k].retrived[1].data;
        for(var n in datas[0]['walkby'].row) {
          dataset_walkby.push(datas[0]['walkby'].row[n]);
        }
        */
        for(var x in d){
          for(var y in d[x].row){
    ...
    if(typeIndex==0){
      return(<View>
                <View style={{flexDirection:'row',
                  ...
                  <Image style={{width:25,height:25,marginLeft:18}} resizeMode={'contain'}
                   source={require('../../images/passengerflow_enter_pic.png')} />
                   <Text allowFontScaling={false} style={{marginLeft:10,fontSize:14,color:VALUES.COLORMAP.gray_font}}>{I18n.t("bi_most_turnin_period")}</Text>
                   <View style={{flex:1}}/>
            ...
            <View style={{flexDirection:'row',
                  ...
                  <Image style={{width:25,height:25,marginLeft:18}} resizeMode={'contain'}
                   ...
                   <Text allowFontScaling={false}  style={{marginLeft:10,fontSize:14,color:VALUES.COLORMAP.gray_font}}>
                      {I18n.t("bi_most_person_period")}
                   </Text>
                   <View style={{flex:1}}/>
                  <Text allowFontScaling={false} style={{fontSize:14,color:VALUES.COLORMAP.gray_font,marginRight:16}}>{this.getBestTime(1).best}</Text>
           </View>
       </View>)
    } else {
      ...
      if(I18n.locale=='en'){
        if(this.state.range=='dd' ){
          t1 ='Highest Daily Turn in Rate'
          t3 ='Lowest Daily Turn in Rate'
        } else if(this.state.range=='ww' ){
          t1 ='Highest Weekly Turn in Rate'
          t3 ='Lowest  Weekly Turn in Rate'
        } else if(this.state.range=='mm' ){
          t1 ='Highest Monthly Turn in Rate'
          t3 ='Lowest Monthly Turn in Rate'
        } else if(this.state.range=='yyyy'){
          t1 ='Highest Yearly Turn in Rate'
          t3 ='Lowest Yearly Turn in Rate'
        }
      } else {
        var t1 = I18n.t("bi_turnin_rate") + I18n.t("bi_do_best") + DataHandler.unitToString(this.state.range)
        var t3 = I18n.t("bi_turnin_rate") + I18n.t("bi_do_worst") + DataHandler.unitToString(this.state.range)
        if(this.state.range=='mm' || this.state.range=='yyyy'){
          t1 =I18n.t("bi_turnin_rate") +I18n.t("bi_do_best")  + DataHandler.unitToString('yyyy')
          t3 = I18n.t("bi_turnin_rate") +I18n.t("bi_do_worst") + DataHandler.unitToString('yyyy')
        }
      }
      var t5,t6;
      if(I18n.locale=='en'){
  ...
  renderChart(){
    console.log("CustomTurnin renderChart");
      ...
      colorbar = (
        <View style={{flexDirection:'row',justifyContent:'center',alignItems:'center'}}>
          <TouchableOpacity onPress={()=>this.setState({visible1: !visible1})} style={{flexDirection:'row',justifyContent:'center',alignItems:'center'}}>
            ...
            <Text allowFontScaling={false} style={{marginLeft:10,color:'#B1B2B4',fontSize:12,opacity: (visible1 ? 1 : 0.5)}}>{I18n.t("bi_turnin_rate")+'%'}</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={()=>this.setState({visible2: !visible2})} style={{flexDirection:'row',justifyContent:'center',alignItems:'center'}}>
            <View style={{marginLeft:24,width:12,height:12,backgroundColor:'#FFC53D',opacity: (visible2 ? 1 : 0.5)}}/>
            <Text allowFontScaling={false} style={{marginLeft:10,color:'#B1B2B4',fontSize:12,opacity: (visible2 ? 1 : 0.5)}}>{I18n.t('bi_peoplecount_in')}</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={()=>this.setState({visible3: !visible3})} style={{flexDirection:'row',justifyContent:'center',alignItems:'center'}}>
            <View style={{marginLeft:24,width:12,height:12,backgroundColor:'#CBCBCB',opacity: (visible3 ? 1 : 0.5)}}/>
            <Text allowFontScaling={false} style={{marginLeft:10,color:'#B1B2B4',fontSize:12,opacity: (visible3 ? 1 : 0.5)}}>{I18n.t('bi_peoplecount_pass')}</Text>
          </TouchableOpacity>
        </View>
      )
    }
    ...
    console.log("CustomTurnin data : " + JSON.stringify(data));

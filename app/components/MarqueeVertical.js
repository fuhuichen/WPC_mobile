...
export default class MarqueeVertical extends Component {
    ...
    singleLineTextView(list){
        let { textStyle, viewStyle, onTextClick, width, height, numberOfLines, headViews, tailViews} = this.props;
        ...
        if(tailViews == null || list == '' || list == [] || list.length <= 0){}else{
            tailViewList = tailViewList.concat(tailViews);
            mTailViewList = mTailViewList.concat(tailViewList);
            mTailViewList.push(tailViewList[0]);
            mTailViewList.unshift(tailViewList[tailViewList.length - 1]);
        }
        ...
        for(let i = 0;i<mlist.length;i++){
            ...
            itemView.push(
                ...
                <View style = {{
                    ...
                    <View style={{flex: 1}}>
                        { mTailViewList ? mTailViewList[i] : null }
                    </View>
                </View>
                </TouchableOpacity>
            );

...
export default class ReactVideo extends Component {
    ...
    componentDidMount() {
        ...
        HttpUtil.get('device/dash/info')
            .then(result => {
                console.log("device/dash/info"+JSON.stringify(result))
                if(result.data){
    ...
    componentWillMount(){
        ...
        this.cancelCreateEmitter = DeviceEventEmitter.addListener('cancelCreateEvent',() =>{
            if(this.state.muted){
    ...
    onAppStateChange = (nextAppState) => {
        if (nextAppState === 'active') {
            if (this.state.realType && !this.state.paused){
    ...
    async startVideo(IVSID, channelId, startTs){
        try {
            if( (this.state.currentState == 'loading' || this.state.currentState == 'play')
                && this.IVSID == IVSID && this.channelId == channelId && startTs == null){
                return;
            }
            this.setState({enableOperate:true});
            if (IVSID === null || channelId === null){
                ...
                this.setState({realType:startTs === null})
                if (startTs){
    ...
    async stopVideo(){
        if(this.sessionId){
            ...
            this.setState({currentState:'blank',error:'',paused:true});
            if (state === 'play'){
    ...
    async stopVideoPlay(){
        if(this.sessionId){
            let state = this.state.currentState;
            this.setState({currentState:'loading',paused:true});
            if (state === 'play'){
                await this.disconnectVideo();
            }
            return await this.offline();
    ...
    async disconnectVideo(){
        if(this.sessionId != null){
            ...
            request.channel = this.channelId;
            data.request = request;
            let url = this.state.realType ? 'LiveStream':'PlaybackStream';
    ...
    async onPlay(){
        ...
        if (this.state.realType === true){
            ...
            if (paused){
                ...
                if(this.state.currentState == 'blank'){
                    this.startVideo(this.IVSID, this.channelId, this.startTs);
                }
                else{
                    if (!this.onEndflag){
    ...
    render() {
        ...
        if ( !this.state.realType ){ backBtn =(
            <View pointerEvents={pointerChannel}>
               <TouchableOpacity onPress={()=> {
                if (this.state.currentState == 'play'){
                    this.setState({paused:false,enableCapture:false});
        ...
        let forwardBtn = null;
        if ( !this.state.realType ){ forwardBtn =(
            <View pointerEvents={pointerChannel}>
               <TouchableOpacity onPress={()=> {
                if (this.state.currentState == 'play'){
                    this.setState({paused:false,enableCapture:false});
        ...
        if (true){ controlBar = (
            <View  pointerEvents={pointer} style={{width:'100%',flexDirection:'row',justifyContent:'space-between',backgroundColor:'black',opacity: 0.7}}>

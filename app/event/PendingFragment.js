...
export default class PendingFragment extends BaseStore {
    state = {
        storeData: [],
    ...
    async fetchData(){
        ...
        if(storeIds == -1) {
            ...
            let eventPending = result.data.filter(p => (p.numOfUnprocessed + p.numOfRejected) > 0);
            if (eventPending.length > 0){
                data.push({title: I18n.t('Event unhandled store'), stores: eventPending, type: [0,3]});
                    ...
                    <View style={styles.group}>
                        {
                            item.stores.map((key,value) =>{
                                return <EventCell data={{key,value}} type={item.type} showDoneLabel={false}/>
                            })
                        }
                    </View>
                </View>

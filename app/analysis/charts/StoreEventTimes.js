...
export default class StoreEventTimes extends Component {
    state = {
    ...
    onRow(item, index){
        ...
        (request.submitters != null) && (filters.clause.assigner = request.submitters);
        (request.status != null) && (filters.clause.status = request.status);
        (request.searchMysteryMode != null) && (filters.searchMysteryMode = request.searchMysteryMode);

        Actions.push('eventList', {filters: filters});
    }
        ...
        return <TouchableOpacity activeOpacity={0.5} onPress={() => this.onRow(item, index)}>
            <View style={styles.data}>
                ...
                <View style={styles.panel}>
                    <Text style={styles.times}>{item.numOfInterest}</Text>

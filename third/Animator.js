...
export default class Animator extends Component{
  ...
  _handlePanResponderMove = (e, gesture) => {
    if (this._swipeInBounds(gesture)) {
      console.log("XX="+(this.props.currentPosition.y + gesture.dy))
      this.position.setValue({ x:this.props.currentPosition.x, y: this.props.currentPosition.y + gesture.dy });
    } else {
        console.log("XXXX="+(this.props.upPosition.y - this._calculateEase(gesture)))
      this.position.setValue({x: this.props.upPosition.x, y: this.props.upPosition.y - this._calculateEase(gesture) });
    }
  }
  ...
  _transitionTo(position, callback) {
    Animated.spring(this.position, {
      toValue: position
    }).start(() =>{console.log("  _transitionTo");});
